...
# Log file for errors
error_log="$HOME/Desktop/sr/results/artemis/error_log.txt"
...
run_benchmark() {
    ...
    # Run the JMH benchmark and save the results to a file
    if ! java -jar -Djmh.ignoreLock=true target/benchmark.jar >>"$output_file"; then
...
for commit_id in "${commit_ids[@]}"; do
    output_file="$HOME/Desktop/sr/results/artemis/${commit_id}.txt"
